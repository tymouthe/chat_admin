      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(master, index) in masters" :key="master.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap">{{ master.name }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ master.username }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ master.password }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ master.lotteryType }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ master.phoneNumber }}</td>
            <td class="px-6 py-4 whitespace-nowrap flex space-x-2">
              <UpdateEntity 
                :entityData="master" 
                :collectionName="'master'" 
                :entityName="'Master'" 
                @onUpdate="fetchMasters" 
                v-slot="{ update }">
...
// State to manage visibility of the form, the list of masters, and loading indicator
...
// Function to fetch masters from Firestore in real-time
const fetchMasters = () => {
  try {
    isLoading.value = true; // Set loading to true before fetching data
    // Set up a real-time listener for changes in the 'master' collection
    onSnapshot(collection(db, 'master'), (snapshot) => {
      masters.value = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      console.log('Realtime Fetched Masters:', masters.value); // Log the fetched data
...
// Fetch masters when the component is mounted
